...
// ─── Color Palettes (Computer Graphics: Color Theory) ───────────────────────
...
function generateLogoConfig(name, options = {}) {
    ...
    // CG: Geometric transformations — rotation is applied in the front-end renderer
    ...
    // CG: Bezier curve control points for decorative arcs
    ...
    // CG: HSL colour blending
    const hslShift = Math.floor(Math.random() * 360);

    // Initials for monogram mode
...
// GET / — health check
app.get('/', (_req, res) => {
    res.json({ status: 'Logo Designer API running 🎨', version: '1.0.0' });
});

// POST /api/generate — generate N logo configs for a name
app.post('/api/generate', (req, res) => {
    const { name, count = 9, filters = {} } = req.body;
    if (!name || typeof name !== 'string' || name.trim().length === 0) {
        return res.status(400).json({ error: 'A logo name is required.' });
    }
    const trimmed = name.trim().slice(0, 50);
    const logos = Array.from({ length: Math.min(count, 12) }, () => generateLogoConfig(trimmed, filters));
    res.json({ logos, name: trimmed });
});

// GET /api/palettes — return available palette types
...
// GET /api/filters — return all filter options
...
app.listen(PORT, () => console.log(`🎨 Logo Designer API → http://localhost:${PORT}`));
